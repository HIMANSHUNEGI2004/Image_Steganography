import cv2 #pip install opencv-python      # Import the OpenCV library for image processing
import os   # Import the os module for interacting with the operating system

class Header:
    #class data members
    NAME_LENGTH = 20          # Maximum length of the file name
    SIZE_LENGTH = 10          # Maximum length of the file size
    HEADER_LENGTH = NAME_LENGTH + SIZE_LENGTH + 8           # Total length of the header
    PAD_CHAR = '*'            # Padding character for file name if it's less than NAME_LENGTH

    @classmethod #Invokable without object
    def generate_header(cls, doc):  # doc =  d:/imp_content/secret.docx
        # Method to generate the header for embedding
        if '/' not in doc:
            raise Exception('Use / as path separator')

        name = doc.split('/')[-1]  # [d:, imp_content, secret.docx]    # Extract the file name from the path

        l = len(name)
        if l > cls.NAME_LENGTH:
            name = name[l - cls.NAME_LENGTH:]  # Trim the file name if it exceeds NAME_LENGTH
        elif l < cls.NAME_LENGTH:
            name = name.rjust(cls.NAME_LENGTH, cls.PAD_CHAR)  # pad

        size = str(os.path.getsize(doc))           # Get the size of the file
        size = size.rjust(cls.SIZE_LENGTH, cls.PAD_CHAR)       # Combine name, size, and delimiters to form the header      
        return '$$$$'+ name + size+ '@AIT'          # Combine name, size, and delimiters to form the header         

    @classmethod
    def validate_header(cls, header):
        #first 4 characters must be '$$$$'
        #and last 4 characters must be '@AIT'
        # Method to validate the header during extraction
        ff = header[:4]
...
class Embeddor:
    #class data
    # Class to handle the embedding process
    ...
    def __init__(self, vessel_image, file_to_embed): #put the object into first state
        #validations
        # Constructor to initialize the Embeddor object with the vessel image and file to embed
    ...
    def embed(self):
        ...
        # Method to embed data into the vessel image
        ...
        h, w, _ = mem_image.shape       # Get the height and width of the image

        # know the size of the document
        doc_size = os.path.getsize(self.file_to_embed)

        # test the embedding capacity
        capacity = h * w          # Calculate the embedding capacity
        ...
        while i < h and flag:  # for each row
            ...
            while j < w and flag:  # for each col of the row i
                ...
                # embed the bits
                red = (red & (~0x7)) | bits[0]      # Embed bits into red channel
                green = (green & (~0x7)) | bits[1]  # Embed bits into green channel 
                blue = (blue & (~0x3)) | bits[2]    # Embed bits into blue channel

                # update the mem_image_pixel[i,j]
                mem_image[i, j, 0] = blue           # Update blue channel
                mem_image[i, j, 1] = green          # Update green channel
                mem_image[i, j, 2] = red            # Update red  channel
